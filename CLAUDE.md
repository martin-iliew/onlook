...
### Designer Onboarding (Mac + Windows)

This repo ships three slash commands for non-technical designers. They work on both Mac and Windows (via Git Bash). When a designer asks for help with setup, startup, or stopping the app — use these commands or follow the same logic.

**Commands (`.claude/commands/`):**
- `/setup <config-url>` — Full first-time setup. Detects OS, installs tools (Homebrew/winget), installs deps, downloads `.env.local` from the config URL shared via Slack, launches the app. Idempotent — safe to re-run. If no URL is passed, Claude asks the user to paste it.
- `/start` — Daily workflow. Starts the dev server, opens the browser.
- `/stop` — Shuts down the dev server cleanly (OS-aware).

**Key facts for manual assistance:**
- Prerequisites (Mac): Homebrew, Git, Node.js, Bun — Prerequisites (Windows): Git, Node.js via winget, Bun via PowerShell
- `.env.local` lives at `apps/web/client/.env.local` — the raw URL is shared privately via Slack (never hardcoded in the repo)
- `bun run dev:cc` starts both the Next.js app (port 3000) and bridge server (port 4567); cross-platform via `scripts/kill-dev-ports.ts`
