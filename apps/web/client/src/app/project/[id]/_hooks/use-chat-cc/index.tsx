...
export function useChat({ conversationId, projectId, initialMessages }: UseChatProps) {
    ...
    // Claude Code bridge needs the sandbox ID to know where to sync files
    const sandboxId = editorEngine.branches.activeBranch?.sandbox?.id;

    const { messages, error, stop, setMessages, regenerate, status } =
        useAiChat<ChatMessage>({
            id: 'user-chat',
            messages: initialMessages,
            // No sendAutomaticallyWhen — Claude Code runs its own tool loop server-side.
            // The bridge streams a single turn and signals finish with finishReason: 'stop'.
